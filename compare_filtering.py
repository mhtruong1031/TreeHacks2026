"""
Try multiple filter configurations against the ground-truth 10_filtered.csv
and plot the best match, including a running divergence over time.

Usage:
    python compare_filtering.py
    python compare_filtering.py --seconds 60
"""
...
# ── Filter helpers (all use SOS for stability) ────────────────────────
...
# ── Candidate filter pipelines to try ────────────────────────────────
...
def score_candidate(name, filt_fn, raw, truth, fs, crop_samples=0):
    """Apply filter to all channels, crop transient, return metrics.

    The filter is applied to the FULL raw signal so the filter has time
    to settle.  Then the first *crop_samples* are trimmed from both the
    filtered output and the truth before computing correlation / errors.

    Returns (mean_corr, per_ch_corr, filtered_full, crop_samples).
    """
    ...
    # Crop transient from both sides for scoring
    ...
    return mean_corr, corrs, out


def running_divergence(ours, truth, fs, window_sec=1.0):
    """Compute RMSE in sliding windows -> (times, per_ch_rmse)."""
    w = int(window_sec * fs)
    n = ours.shape[0]
    n_ch = ours.shape[1]
    steps = n // w
...
def main(raw_path, filtered_path, fs, seconds, crop_sec):
    print("Loading CSVs (this takes a moment for 1.28M rows)...")
    ...
    # ── Detailed metrics (cropped) ────────────────────────────────────
    bd = best_data[crop_samples:]
    tr = truth[crop_samples:]

    print(f"\n  Best filter detailed metrics (after {crop_sec}s crop):")
    ...
    # ── Plot (all plots use cropped data) ─────────────────────────────
    ...
    if n_ch == 1:
        axes1 = [axes1]
    fig1.suptitle(f"Best: {best_name}  (5s after {crop_sec}s crop)", fontsize=13)
    ...
    # Figure 2: running RMSE divergence (cropped)
    ...
    if n_ch == 1:
        axes2 = [axes2]
    fig2.suptitle(f"Running RMSE vs Ground Truth (1s windows, after {crop_sec}s crop)", fontsize=13)
    ...
    # Figure 3: point-wise difference (cropped)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--crop", type=float, default=3.0,
                        help="Seconds to crop from the start to skip filter transient (default 3)")
